...
The system SHALL provide REST endpoints to create, read, update, and delete assets.

#### Scenario: List all assets
- **WHEN** a GET request is made to `/assets`
- **THEN** the system returns a list of all assets with their details (id, name, type, etc.)
...
- **THEN** the system returns the asset details if found, or 404 if not found
